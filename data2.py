...
class TrainDataset(Dataset):
    def __init__(self, src_path, tgt_path, tokenizer, block_size, wwm):
        ...
        for i, (__text1, __text0) in enumerate(zip(tqdm.tqdm(fsrc), ftgt)):
            text0 = __text0.strip()
            text1 = __text1.strip()
            ...
            if wwm:
                mask_ids = make_mask(text0, tok0, len(input_ids), tokenizer)
            else:
...
class EvalDataset(Dataset):
    def __init__(self, src_path, tgt_path, tokenizer, block_size, wwm, N, M):
        ...
        for i, (__text1, __text0) in enumerate(zip(fsrc, ftgt)):
            ...
            if wwm:
                mask_idx = make_mask(text0, tok0, len(__input_ids), tokenizer)[1 : len_0 + 1]
